...
"""
This is the slightly reworked script
from `transformers/examples/language-modeling/run_clm_no_trainer.py` (v4.34.1)
with an additional step of distillation.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--use_layer_reduction",
        action="store_true",
        help="Init the student model using layers of teacher.",
    )
...
def main(args: argparse.Namespace):
    ...
    if accelerator.is_main_process:
        if args.output_dir is not None:
            ...
            if os.listdir(args.output_dir) and not args.overwrite_cache:
                raise ValueError(
                    "Output directory ({}) already exists and is not empty. Use --overwrite_cache to overcome.".format(
                        args.output_dir
                    )
                )
    ...
    (
        ...
        train_dataloader,
        eval_dataloader,
        # device_placement=[False, True] + [True] * 4,
    )
